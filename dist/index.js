class MarkdownUtil {
    constructor(textarea) {
        if (!textarea)
            throw new ReferenceError('Missing textarea parameter');
        if (!(textarea instanceof HTMLTextAreaElement) && !(textarea instanceof HTMLInputElement))
            throw new TypeError('textarea must be HTMLTextAreaElement or HTMLInputElement');
    ...
    addAttributeSelection(options) {
        ...
        if (!splitText?.length)
            return null;
        splitText.forEach((c, i) => {
            if (i === startSelection && options.attributes.start)
                splitText[i] = `${options.attributes.start}${c}`;
            if (endSelection && i === endSelection - 1 && options.attributes.end)
                splitText[i] = `${c}${options.attributes.end}`;
        });
        return splitText.join("");
    }
    ;
    addAttributePosition(options) {
        console.log(options);
        if (options.positions.start === "current" && options.positions.end === "current") {
            const startSelection = this.textarea.selectionStart;
            if (!startSelection)
                return (this.textarea.value += `${options.attributes.start ? options.attributes.start : ""}${options.attributes.end ? options.attributes.end : ""}`);
            const splitText = this.textarea.value.split("");
            if (this.textarea.value.length === startSelection)
                return this.textarea.value += `${options.attributes.start}${options.attributes.end}`;
            splitText.forEach((c, i) => {
                if (i === startSelection)
        ...
        if (options.positions.start === "startLine" && options.positions.end === "startLine") {
            ...
            if (start || start === 0) {
                ...
                splitText.forEach((c, i) => {
                    ...
                    if (start == i)
                        splitText[i] = `${options.attributes.start}${c}${options.attributes.end}`;
                });
                return splitText.join("");
            }
        }
